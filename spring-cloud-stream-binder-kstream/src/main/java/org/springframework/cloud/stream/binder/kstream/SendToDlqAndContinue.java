...
public class SendToDlqAndContinue implements DeserializationExceptionHandler{
	...
	public void sendToDlq(String topic, byte[] key, byte[] value, int partittion, Headers headers){
		KStreamDlqDispatch kStreamDlqDispatch = dlqDispatchers.get(topic);
		kStreamDlqDispatch.sendToDlq(key,value, partittion, headers);
	}

	@Override
	public DeserializationHandlerResponse handle(ProcessorContext context, ConsumerRecord<byte[], byte[]> record, Exception exception) {
		KStreamDlqDispatch kStreamDlqDispatch = dlqDispatchers.get(record.topic());
		kStreamDlqDispatch.sendToDlq(record.key(), record.value(), record.partition(), null);
		context.commit();
