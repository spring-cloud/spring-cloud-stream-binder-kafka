[[overview]]
--
This section goes into details about how you can work with Spring Cloud Stream Kafka binder.
--

== Introduction
This is Kafka specific implementation for Spring Cloud Stream. This implementation supports Kafka versions higher than 0.9.

==== Classpath Detection

By default, Spring Cloud Stream relies on Spring Boot's auto-configuration to configure the binding process.
Following is the maven cordinates for including the Kafka binder in a Spring Cloud Stream application.
[source,xml]
----
<dependency>
...
For common configuration properties pertaining to binder,
