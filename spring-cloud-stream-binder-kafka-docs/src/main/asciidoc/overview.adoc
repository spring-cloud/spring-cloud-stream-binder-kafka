...
== Kafka Stream Binding Capabilities of Spring Cloud Stream
...
For more information on Kafka Streams, see https://kafka.apache.org/0110/documentation/streams/developer-guide[Kafka Streams API Developer Manual]
...
The properties must be prefixed with `spring.cloud.stream.kstream.binder.`.

streamConfiguration::
  Map with a key/value pair containing properties pertaining to Kafka Streams API.
  For all the properties that may go into streams configuration, see http://www.trieuvan.com/apache/kafka/0.10.2.1/javadoc/org/apache/kafka/streams/StreamsConfig.html[StreamsConfig javadoc].