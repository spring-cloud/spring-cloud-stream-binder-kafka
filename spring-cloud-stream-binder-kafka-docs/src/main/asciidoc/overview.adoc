...
If set to `false`, an `org.springframework.kafka.support.Acknowledgment` header will be available in the message headers for late acknowledgment.
In this case, the key for the header will be `kafka_acknowledgment`.
