...
Kafka Streams binder provides an API that allows you to wrap the critical sections of the business logic in retrying temlate.
The API is `CriticalPathRetryingKafkaStreamsDelegate` and applications can inject this and use it within the processor.
Following are some examples of using this.

First, we show the usage of the API method that can only be used as a terminal operation.
This means that, once the method is called, your processing is complete and no further operations can be continued.
An example of a situation where this is useful is when you need to receive a stream of data, do some transformations and then insert the result into a relational database.
            ...
            .process(() -> new Processor<Object, String>() {
                ...
                public void process(Object o, String s) {
                    criticalPathRetryingKafkaStreamsDelegate.retryCriticalPath(() -> {
                        //critial business logic goes here.
                    });
                }
...
As this is a termainl operation, it is recommended to use this API with low level Processor API in Kafka Streams.

Here is another example of using a different API from this class, but this time it allows you to use it on non-termainal operation.
It provides a result, using which further processing can continue.

```
@Bean
public java.util.function.Function<KStream<Object, String>, KStream<Object, String>> functionProcess() {
...
As we noted above, the result of the `transform` call is used for further processing (the `map` call in this case).

In both of these instances, the code that is delegated to the API as critical section gets retried.
Here are the binder properties that you can use to configure the retries.

`spring.cloud.stream.kafka.streams.binder.critical-path-retry.max-attempts` (Default is `1`).
`spring.cloud.stream.kafka.streams.binder.critical-path-retry.backoff-period` (Default is 1 second)
...
+
Default: 1000 ms
critialPathRetry.maxAttempts::
Max attempts for how many times a critical section in the application gets retried using `CriticalPathRetryingKafkaStreamsDelegate`.
+
Default: 1

stateStoreRetry.backoffPeriod::
Backoff period when retrying a critical section using `CriticalPathRetryingKafkaStreamsDelegate`.
+
Default: 1000 ms
