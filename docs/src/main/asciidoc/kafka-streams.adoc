...
In the case of multiple input bindings, there will be a separate `RetrtyTemplate` bean available per binding.
...
OOnce the `RetryTemplate` from the binding is injected into the application, it can be used to retry any critical sections of the application.
Here is an example:
