...
A quick way to bootstrap a new project for Kafka Streams binder is to use http://start.spring.io[Spring Initilizr] and then select "Cloud Streams" and "Spring for Kafka Streams" as shown below

image::{github-raw}/docs/src/main/asciidoc/images/spring-initializr-kafka-streams.png[width=800,scaledwidth="75%",align="center"]

=== Overview

Spring Cloud Stream's Apache Kafka support also includes a binder implementation designed explicitly for https://kafka.apache.org/documentation/streams/[Apache Kafka Streams] binding.
...
Kafka Streams binder implementation builds on the foundations provided by the https://docs.spring.io/spring-kafka/reference/html/_reference.html#kafka-streams[Spring for Apache Kafka] project
...
Spring Cloud Stream processor model can be directly mapped to this.
It can also be used in Processor applications with no-outbound destinations.

In the following sections, we are going to look at the details of Spring Cloud Stream's integration with Kafka Streams.

=== Programming Model

Using the programming model provided by Kafka Streams binder, both https://docs.confluent.io/current/streams/developer-guide/dsl-api.html[highe level Streams DSL] and the lower level https://docs.confluent.io/current/streams/developer-guide/processor-api.html[processor models] can be used.

==== Functional Style

Kafka Streams binder allows the applications to be written using the functional support added as part of Java 8.
...
Albeit simple, this is a complete stand alone Spring Boot application that is leveraging Kafka Streams for stream processing.
This is a consumer application with no outbound binding and only a single inbound binding.
The application consumes data and do some terminal operations on that data.
...
Kafka Streams binder for Spring Cloud Stream automatically creates the binding behind the scenes.
...
This is the classic word count example in which the application receives data from a topic and then count the number of occurrences for each word in a time window.
...
Here again, this is a complete Spring Boot application. The difference from the first application is that the bean method is of type `java.util.function.Function`.
...
As in the first `Consumer` based app, the input binding is named as `process_in` by default. For the output, the binding name is set to `process_out`.
...
The two examples we saw above have a single `KStream` input binding. In both cases, the bindings received the records from a single topics.
...
For many non trivial Kafka Streams applications, it is often necessary to consume data from more than one topic through multiple bindings.
...
Maybe the underlying topic is populated through a change data capture (CDC) mechanism from a database or the application only cares about the latest updates for the records etc.
...
Here again, the basic theme is the same as above, the difference being now you have two inputs and are using Java's `BiFunction` support to bind the inputs to the destinations.
...
Kafka Streams binder make use of this feature to enable multiple input bindings.
...
This output function's input the third input for the application (`Product`) and its output is a `KStream` which is output binding for the application.
...
Therefore if your Kafka Streams application requires more than a reasonable number of input bindings and you want to use this functional model, then you may want to rethink your design and decompose the application appropriately.
...
*In the case of more then one output, the type simply becomes `KStream[]`.
...
As you can see, this is a bit more verbose, as you need to provide `EnableBinding` and the other extra annotations like `StreamListener` and `SendToe`.
...
You might notices that the above two examples are even more verbose since in addition to provide `EnableBinding`, you also need to write your own custom binding interface as well.
...
In this secion, we saw the various ways the Kafka Streams binder enables the applications to be written in vaious programming styles.

The binder provides bining capabilities for `KStream`, `KTable` and `GlobalKTable` on the input.
...
If you only have one single Kafka Streams function or `StreamListener` in the application, then you can set this at the binder level using the following property:

`spring.cloud.stream.kafka.streams.binder.applicationId`.

As a convenience, if you only have a single function or `StreamListener`, you can also use `spring.application.name` as the property to delegate the application id.

If you have multiple Kafka Streams functions or `StreamListener`s in the application, then you need to set the application id per function or listener.
You need to pick the first input binding of each function or listener and set application id on that binding.
...
Then you must set the application id, using the following properties.
...
In the case `StreamListener`, instead of using the function bean name, the generated application ID will be use the containing class name followed by the method name.

Here is the order of precedence for application ID in a nutshell (from lowest to highest)

* Auto generated by the binder
* spring.kafka.streams.applicationId or spring.application.name (Can only be used if you have a single processor in the application)
* binder level (spring.cloud.stream.binder.kafka.streams.applicationId) (only use if you have single processor)
* binding level (spring.cloud.stream.kafka.streams.bindings.process_in.applicationId _change process_in accordingly_.)

=== Overriding default input/output binding names generated by the binder when using the functional style.

By default, the binder uses the strategy laid out above to generate the binding name when using the functional style, i.e. <function-bean-name>_<in>|<out>_[0..n], for e.g. process_in, process_in_0 etc.
...
Now, you want to change them to something else completely, maybe more domain specific binding names. You can do so, as below.
...
Kafka Streams binder will first check if Kafka Streams binder specific broker property is set (`spring.cloud.stream.kafka.streams.binder.brokers`) and if not found looks for `spring.cloud.stream.kafka.binder.brokers`.
